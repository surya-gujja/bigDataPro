...
\begin{itemize}
  ...
  \item \textbf{Raw Data Sample}: \texttt{data/metrics\_sample.csv} stores the first hundred metric
        rows.
  \item \textbf{Packaging Scripts}: The repository omits generated zip archives. Recreate
        \texttt{cljDetector.zip} and \texttt{MonitorTool.zip} using the instructions in each
        component's README when preparing the final submission bundle.
\end{itemize}
